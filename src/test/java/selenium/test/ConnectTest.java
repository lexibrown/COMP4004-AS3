...
public class ConnectTest extends AbstractSeleniumTest {
	 ...
	 * driver = new FirefoxDriver(); baseUrl = Variables.baseUri;
	 ...
	 * // driver.navigate().to(Variables.baseUri + Endpoint.Connect.CONNECT +
	 * "?user=test1"); WebElement webElement =
	 * driver.findElement(By.id("content")); String token =
	 * webElement.getText();
	 * 
	 * System.out.println(token);
	 * 
	 * @SuppressWarnings("unchecked") Map<String, Object> map =
	 * JsonUtil.parse(token, Map.class); System.out.println(map);
	 * 
	 * assertTrue(map.containsKey(Key.TOKEN));
	 * assertNotNull(map.get(Key.TOKEN)); }
	 * 
	 ...
	 * StringBuffer result = new StringBuffer(); HttpClient client = new
	 * DefaultHttpClient(); String url = Variables.baseUri +
	 * Endpoint.Connect.CONNECT + "?user=test1"; HttpGet request = new
	 * HttpGet(url); request.addHeader("User-Agent", USER_AGENT); HttpResponse
	 * response = client.execute(request); int responseCode =
	 * response.getStatusLine().getStatusCode();
	 * System.out.println("Response Code: " + responseCode); try { if
	 * (responseCode == 200)
	 * 
	 * { System.out.println("Get Response is Successfull"); BufferedReader
	 ...
	 * 
	 * } catch (Exception ex) { result.append("Get Response Failed"); return
	 * result.toString(); }
	 * 
	 * } }
	 */
