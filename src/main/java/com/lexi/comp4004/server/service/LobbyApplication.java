...
public class LobbyApplication {
	...
	public String getUsers(HashMap<String, Object> params) {
		try {
			...
			return JsonUtil.makeJson(Key.LOBBY, JsonUtil.stringify(Lobby.getInstance().getUsers()));
		} catch (Exception e) {
	...
	public String lobbyStatus(HashMap<String, Object> params) {
		try {
			...
			return JsonUtil.makeJson(Key.STATUS, Lobby.getInstance().isGameInSession());
		} catch (Exception e) {
			return JsonUtil.fail(e);
		}
	}
	...
	public String joinGame(HashMap<String, Object> params) {
		try {
			...
			if (Lobby.getInstance().joinGame(params.get(Key.TOKEN).toString())) {
				return JsonUtil.makeMessage("Successfully joined game.");
			}
			return JsonUtil.errorJson(SERVICE + "-4002",
					"Failed to join game. Please ensure that a game is set up and that it isn't full.");
		} catch (Exception e) {
			return JsonUtil.fail(e);
		}
	}
	...
	public String startGame(HashMap<String, Object> params) {
		try {
			...
			if (Lobby.getInstance().startGame(params.get(Key.TOKEN).toString())) {
				return JsonUtil.makeMessage("Successfully started game.");
			}
			return JsonUtil.errorJson(SERVICE + "-5002",
					"Failed to start game. Please ensure enough players have joined and you are a part of the game.");
		} catch (Exception e) {
			return JsonUtil.fail(e);
		}
	}
	 ...
	 * getUsersHtml(@QueryParam(TOKEN) String token) { try { if
	 * (token.isEmpty()) { return JsonUtil.errorJson(SERVICE + "-1000",
	 * "No token provided."); }
	 * 
	 * List<String> users = Lobby.getInstance().getUsers();
	 * 
	 * if (users.isEmpty()) { // this should never display return
	 * "<html><title>COMP4004-Poker-Lobby</title><body><h1>No active users in the lobby.</h1></body></html>"
	 * ; }
	 * 
	 * String html =
	 * "<html><title>COMP4004-Poker-Lobby</title><body><h1>Users:</h1><h2>"; for
	 * (String u : users) { html += u + "\n"; }
	 * 
	 * return html + "</h2></body></html>";
	 * 
	 * } catch (Exception e) { return JsonUtil.fail(e); } }
	 * 
	 * @GET
	 * 
	 * @Path(STATUS)
	 ...
	 * 
	 * @Produces(MediaType.TEXT_HTML) public String
	 * lobbyStatusHtml(@QueryParam(TOKEN) String token) { try { if
	 * (token.isEmpty()) { return JsonUtil.errorJson(SERVICE + "-1000",
	 * "No token provided."); }
	 * 
	 * if (Lobby.getInstance().isGameInSession()) { return
	 * "<html><title>COMP4004-Poker-Lobby</title><body><h1>A game is currently in session.</h1></body></html>"
	 * ; } return
	 * "<html><title>COMP4004-Poker-Lobby</title><body><h1>No game currently in session.</h1></body></html>"
	 * ; } catch (Exception e) { return JsonUtil.fail(e); } }
	 */
